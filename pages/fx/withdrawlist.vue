			...
			<view class="tab-content">
				...
				<view v-if="orderList.length>0">
					...
					<view 
						...
						class="order-item" @tap="navTo(item.id)"
					>
						<view class="i-top b-b">
							<text class="time">订单号:{{item.orderSn}}</text>
							<text class="state" :style="{color: item.stateTipColor}">{{item.orderStatusText}}</text>
							<text
								v-if="item.state===9" 
						...
						>
							...
							<view class="right">
								...
								<text class="attr-box">{{goodsItem.goodsName}}  x {{goodsItem.number}}</text>
				...
				<view class="ectouch-notcont" style="margin-top:130upx" v-else>
				    ...
				    <text class="cont">亲,暂无当前状态的订单哦</text>
				</view>
	...
	export default {
		...
		methods: { 
			...
			loadData(flag){
				...
				let orderurl = '/order/list?showType='+ this.tabCurrentIndex +'&page='+this.currentpage+'&limit=10'
				api.httpget(orderurl).then((res) => {
					let respons = res.data
					if (respons.errno == 0) {
						if(this.refresh) this.orderList = []
						this.orderList = this.orderList.concat(respons.data.list)
