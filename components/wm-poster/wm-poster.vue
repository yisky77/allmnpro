<template>
	<view>
		...
		<view @tap="saveImg" class="saveImg">保存图片</view>
	</view>
</template>
		...
		getNewImage(){
			uni.canvasToTempFilePath({
				...
				complete: (res) => {
					...
					_this.imgSrcurl = res.tempFilePath;
					// _this.$emit('success',res);
				}
			},this);
		},
		saveImg() {
			_this.$emit('save',_this.imgSrcurl);
		}
	},
	mounted() {
		_this = this;
...
<style>
	.saveImg{
		width:80%;height:80upx;position: absolute;bottom:20upx;left:0;right:0;margin:auto;background: #fa436a;border-radius: 50upx;text-align: center;line-height: 80upx;
